...
MapState Map::validate() const
{
    ...
    // Scan through territories via continents and flag the error when a territory is seen twice
...
{
    if (continentId <= 0 || static_cast<std::size_t>(continentId) > this->continents->size())
    {
